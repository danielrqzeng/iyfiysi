...
type ProjectFileType struct {
	...
	Params []TemplateParamsType `json:"params"` //参数，kv格式
}
...
//TemplateFileList 文件模板列表
//此处为何不做yaml配置来呢？主要还是限制下不要让其太灵活，太浪会闪腰
var TemplateFileList []*ProjectFileType = []*ProjectFileType{
	//cmd/gateway/build.sh
	{
		GatewayBuildSh,
		TemplateConfigTypeFile,
		"",
		"",
		filepath.Join("gateway_build.sh.tmpl"),
		filepath.Join("cmd", "gateway", "build.sh"),
		[]TemplateParamsType{},
	},
}
